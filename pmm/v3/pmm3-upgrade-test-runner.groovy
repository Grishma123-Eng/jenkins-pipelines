library changelog: false, identifier: 'lib@PMM-7-fix-upgrade', retriever: modernSCM([
    $class: 'GitSCMSource',
...
pipeline {
    ...
    parameters {
        string(
            defaultValue: 'PMM-7-fix-upgrade-job',
        ...
        stage('Run pre upgrade UI tests') {
            steps {
                withCredentials([aws(accessKeyVariable: 'BACKUP_LOCATION_ACCESS_KEY', credentialsId: 'BACKUP_E2E_TESTS', secretKeyVariable: 'BACKUP_LOCATION_SECRET_KEY'), aws(accessKeyVariable: 'AWS_ACCESS_KEY_ID', credentialsId: 'PMM_AWS_DEV', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY')]) {
                    sh '''
                        ./node_modules/.bin/codeceptjs run-multiple parallel --reporter mocha-multi -c pr.codecept.js --steps --grep '@pre-upgrade'
                    '''
                }
            }
        }
