...
pipeline {
    ...
    post {
        ...
        success {
            script {
                if (params.install_repo == "main") {
                    echo "install_repo=main → Triggering site check job..."

                    dir("package-testing-version-reader") {
                    git poll: false, branch: "master", url: "https://github.com/Percona-QA/package-testing.git"
                    def versionFile = readFile("VERSION")

                    // Select correct version key based on product_to_test
                    def key = ""
                    if (product_to_test == "ps_80")       key = "PS80_VER"
                    else if (product_to_test == "ps_84")  key = "PS84_VER"
                    else if (product_to_test == "ps_57")  key = "PS57_VER"
                    else if (product_to_test == "ps_lts_innovation") key = "PS_INN_LTS_VER"

                    def line = versionFile.readLines().find { it.startsWith(key + "=") }
                    if (line) {
                        // Remove key + = and strip quotes
                        env.FULL_VER = line.split("=")[1].trim().replace("\"", "")
                    }
                }

                    if (!env.FULL_VER?.trim()) {
                        error " ERROR: Could not extract version from VERSION file for ${product_to_test}"
                    }
                    echo " full_ver to pass downstream = ${env.FULL_VER}"

                // Step 2 — Trigger downstream job with 3 parameters
                    build job: 'ps-site-check',
                        wait: false,
                        parameters: [
                            string(name: 'PS_VER_FULL', value: env.FULL_VER),
                            string(name: 'TESTING_BRANCH', value: 'ps-site-check'),
                            string(name: 'TESTING_GIT_ACCOUNT', value: 'grishma123-eng')
                        ]
                } else {
                    echo "install_repo is not 'main' → Skipping downstream trigger."
